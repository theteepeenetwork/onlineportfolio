...
| **R14** | A parent's sign-in link reaches the wrong person, or an email discloses a child | Sending is via **Brevo** (EU: France/Germany), which receives the adult address only. **No child's name and no child content appears in any email** — templates are written so a misdirected message tells a stranger nothing about a child, which matters because the *school* holds the address and schools mistype them. **Storyjar cannot tell whether a particular parent opened an email**: every send asks Brevo to disable tracking, and Brevo's account-level *anonymous email tracking* is on, so any open or click Brevo records is not associated with an individual. *(Corrected 2026-08-16. This row previously said "opens are not tracked". Brevo's transactional log for 16 August 2026 at 14:04 shows a first-opening event recorded against a magic-link email that the application sent with `disableTracking: true`, so the API flag alone does not prevent an open being recorded and attributed. The account-level anonymity setting is what supports the claim now, and the claim is deliberately the narrower one. See `scripts/verify-mail.mjs` and `scripts/brevo-events.mjs`.)* Click-tracking must never occur, because rewriting the link would break the token and hand a third party the means to use it. **That one is checked, not assumed**, precisely because we now know the API flag can be ignored: `scripts/verify-mail.mjs` sends a probe carrying a known `storyjar.co.uk` link, and the delivered message source is read to confirm the href was not rewritten to a provider redirect domain. *(Status 2026-08-16: probe sent and delivered; source inspection outstanding. If links are ever found rewritten, that is a functional defect in the parent sign-in path, not a wording problem, and it gets its own entry in `FINDINGS.md`.)* Brevo's delivery logs are deleted after **1 month** and **message previews are switched off**, so a live sign-in link is never stored in Brevo's interface. Tokens are never logged by us, are single-use, and expire in 30 minutes. | **Low** |
