...
**Version:** 1.3 (draft for sign-off) · **Written:** 15 August 2026 · **Updated:** 7 September 2026 (**R18, parent–teacher messages held to school-set office hours — this one DOES add a category of personal data**, the first since the assessment was written, and a second purpose in §2.4; R6 closed on Railway's written confirmation that backups sit in the service's own region; previously 17 August 2026 for R15, the platform operator identity, the operator read screens with their lookup records, the read-only billing view, and the first two named operator actions, none of which added a category; 16 August 2026 for R14; 15 August 2026 for R6)
...
the school can see the shape of but not the content (§4, R18). Nothing else.
...
| R18 | Keep parent messaging off by default and school-switched; keep the office-hours hold two-way and override-free; keep the three message tables in the operator gate's strictest class and queryable from one module only | Sustained | Founder | Ongoing — each is a blocking test |
| R18 | Put the parent-facing wording (not for emergencies; who can read it; when a held message arrives) in front of pilot parents in the usability kit, and ask a pilot school's DSL whether "close the conversation" is enough or whether a designated-lead read with a recorded reason is needed — a separate amendment if so | Reduced | Founder | During the pilot phase |
| R18 | Say in the privacy notice for families, in the same plain English as the rest of it, that a parent can now write to the teacher, who can read it, and that nothing is emailed | Transparency | Founder | Before a real parent can send one |
