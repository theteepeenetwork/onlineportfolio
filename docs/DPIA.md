...
**Version:** 1.4 (draft for sign-off) · **Written:** 15 August 2026 · **Updated:** 8 September 2026 (**R20, permission slips — a new category of personal data, and the first one whose whole design is about a category it must NOT collect**: the school writes the question and the two answers are a constant in the code, so there is no route by which health, dietary or SEN information can be typed in; the school office sees counts and never a child); previously 7 September 2026 (**R19, parent–teacher messages held to school-set office hours — this one DOES add a category of personal data**, the first since the assessment was written, and a second purpose in §2.4; R6's backup residual closed on Railway's written confirmation that backups sit in the service's own region (F35); previously 17 August 2026 for R15, the platform operator identity, the operator read screens with their lookup records, the read-only billing view, and the first two named operator actions, none of which added a category; 16 August 2026 for R14; 15 August 2026 for R6)
...
| **Parent–teacher messages** (added 7 September 2026) | **Child** (the subject of every message), and the adults who write them: the child's linked parents and named school staff | One conversation per child, text only, between the child's linked parents and the class teacher — or a colleague the teacher has shared it with or passed the family to. A message is about a named child and routinely names them, so the three tables that hold conversations are in the operator gate's strictest class and the operator area cannot read them. Held to office hours the **school** sets inside StoryJar's caps; nothing is delivered outside them in either direction. The school admin sees who is in a conversation and whether anyone is waiting, never a word of it. Off by default; school plan only; nothing emailed. Retention in `RETENTION.md` ("Parent–teacher messages"); risk assessment at **R19**; the rule at `SAFEGUARDING.md` 21. |
| **Permission slips** (added 8 September 2026) | **Child** (the slip is about them), and the parent who answers | A school's form to whole classes — its title and its own words — and one row per child holding **which of two fixed answers** their household gave, an optional packed-lunch flag, which household answered and when. **The school never writes an answer.** The answer set is a constant in the code — *I give permission* / *I do not give permission* — and there is **no free-text response field in the product**, which is what keeps the "no special category data" position below true by construction rather than by asking a school to be careful. The one extra answer permitted is *"my child needs a packed lunch provided"*, a catering headcount for a trip day; it says nothing about what a child may eat. Every form carries a line telling the family that anything medical, dietary or about how their child is looked after goes to the school office. The response table is in the operator gate's strictest class; the form itself holds no person. The school office sees **counts per class and never a child**; which child answered which way is the class teacher's register. Retention in `RETENTION.md` ("Permission slips"); risk assessment at **R20**; the rule at `SAFEGUARDING.md` 22. |
| **A school's parent-messaging settings** (added 7 September 2026) | Nobody identifiable, plus per member of staff whether the school lets them message families | The switch, the hours, closed days. Adult configuration; no child in it. |
...
own family. And, from 7 September 2026, one second purpose, bounded as tightly
as the first: to let that child's family and their class teacher write to each
other about that child, inside office hours the school sets, in a conversation
the school can see the shape of but not the content (§4, R19). Nothing else.
The data is **never** repurposed for analytics,
...
- **Parent–teacher messages, and why the position above survives them** (added
  7 September 2026). The paragraphs above argue that StoryJar deliberately built
  no channel to a parent. It now has one, and it is worth saying exactly what
  did and did not change. **Nothing about contact details changed:** StoryJar
  still holds no address for a parent unless the parent typed it, and a message
  is not sent to an address — it is shown inside the family space the parent
  already signs into, and **no email or notification of any kind is sent about
  it** (rule 6a). The family's badge counts what arrived since they last looked.
  What changed is that a parent can now *write*, and the necessity test for that
  is the one a school would apply to its own office: contact between a family
  and a class teacher about a child is a normal, expected part of the school's
  educational task, and the alternative is the channels schools use today —
  staff mobile numbers, a third-party app running at eleven at night — which
  the school neither governs nor sees.

  So the design's job was to be the *more* proportionate version of that, and
  the constraints are the proportionality argument (each is a blocking test):
  it is **off until a school admin switches it on**; it is **delivered only
  inside office hours the school sets, in both directions, inside StoryJar's own
  caps of ten hours a day between 06:00 and 20:00, with no override**; it is
  **text only**, so no media and no attachments; it is **about one child and
  reachable only by that child's linked parents and named staff of that
  school**, each of whom the parent can see listed; **no child can read or write
  it**; it is **never emailed**; the **school admin sees metadata and never a
  body**; the **operator cannot read it at all**; and it is **audited without
  being quoted**. A teacher who would rather not correspond with one family
  passes the conversation to a colleague, and the family is not told, because
  a family's channel to the school is not the place to record a member of
  staff's decision about them. The text of a message is free text written by an
  adult about a child, which is the R15 problem at far greater volume: nothing
  can stop a parent typing anything, and the controls are who can read it, for
  how long, and that it is never copied anywhere else.
...
| 5. Detrimental use of data | Data is used only to show a child's work to their teacher and their own family, and (from 7 September 2026) to carry a conversation between that family and that teacher about the child — never for anything else. | — |
...
| 11. Parental controls | Parents see their own child's approved work, read-only, and (from 7 September 2026) may write to the child's class teacher inside the school's office hours — the one thing a parent can write, and it is never seen by the child. No covert monitoring of the child by anyone. Access is granted by the school one child at a time, and the school can re-issue or withdraw it in one tap; the parent controls their own contact details and can remove them. | — |
...
| **R6** | Personal data processed outside the UK/EEA | **Hosting region confirmed 2026-08-15: EU West (Amsterdam, Netherlands)**, with the data volume in the same region — so children's moments, media and account data are held in the EEA, satisfying rule 10. UK→EEA transfers are covered by the UK's adequacy regulations, so no IDTA or SCCs are required for the storage location. Stripe holds **adult billing data only**; no child data ever reaches it. | **Medium.** Three residuals: (a) Railway is **US-incorporated**, so its personnel may access systems for support from outside the EEA — its DPA and onward-transfer terms have not been obtained and recorded; (b) **Stripe's billing-data residency is still unassessed**; (c) ~~the geographic location of the volume backups is not confirmed~~ — **closed 2026-09-05 (F35)**: Railway answered the direct question in writing, and volume backups are stored in the same region as the service, **EU West (Amsterdam)**. The hosting region confirmed above covers the database and media at rest and never established where a backup of them was held, which is why this stayed open for nineteen days on a complete second copy of every child's work. `RETENTION.md` had removed the Amsterdam claim rather than repeat it, and instructed that the privacy notice must not claim EU backups until the question was answered in writing; it now has been, and the notice says so. Neither (a) nor (b) involves children's data at rest leaving the EEA; (c) is unknown rather than adverse. |
...
| **R19** | A parent–teacher message about a child is read by the wrong person, reaches a child, arrives when a teacher is off duty, or is quoted somewhere with a longer life than the conversation *(added 7 September 2026)* | **Who can read it** is resolved on the server from the session and the child, never from an id on a form: the child's linked parents on one side (the same parent↔child link as R11), and on the other the class teacher via the class, a colleague the teacher shared it with, or the colleague a teacher passed the family to — each only while still staff of that school and only if the school's per-staff switch allows them to message families. Cross-tenant, colleague's-class, admin-without-a-class and tampered-form routes all get the same 404 or the same generic refusal and create no row (`messaging-cross-tenant.spec.ts`, `messaging-sharing.spec.ts`, `messaging-handover.spec.ts`). **No child** has any route to it: no student session reads the tables and nothing renders in the child's product. **Off duty** is the point of the feature: delivery is a stored time computed from the *school's* hours inside StoryJar's caps, checked on every read the other side gets, re-timed when the hours change, with no override in code (`messaging-office-hours.spec.ts`, `office-hours-rules.spec.ts` including both clock-change nights). **Longer life elsewhere:** the audit log records that a message was sent and by whom, never its text (`messaging-admin-blindness.spec.ts`); nothing is emailed (rule 6a); the operator area cannot read the three tables (gate class CREDENTIAL_NEVER, `bad-message-read.txt`); only one module may query them (`audit-static.mjs`). The **school admin** sees metadata — who reads a thread, how many messages wait and for how long — and can close, share or reassign a conversation without a body ever reaching the console, including an admin who also teaches (`messaging-admin-blindness.spec.ts`). A teacher's reason for passing a family on is optional, lives on the thread row, and never reaches the audit log or the parent. | **Low–Medium.** Low on every named route. What holds it off plain Low is the same thing as R15: the body is free text written by an adult about a child, and nothing can stop a parent typing anything into it — a medical detail, another child's name, an allegation. The controls are that it is seen only by the people above, kept only as long as the child's record, never copied, and that the school can close a conversation at once; the residual is recorded rather than argued away. Backups are now EU-resident (R6, 7 September 2026), so a message shares the jar's position there. |
| **R20** | A permission slip becomes the route by which special category data about a child — an allergy, a medical need, a SEN status, a religion — is collected, or an answer about a named child is read by somebody with no reason to see it *(added 8 September 2026)* | **The collection risk is closed structurally, not procedurally.** A school writes the QUESTION; the ANSWERS are a two-value constant in `src/lib/consent.ts`, and there is no free-text response field on a form anywhere in the product — no control on the builder for adding an option, and none in the schema for storing one. There is therefore no route by which "nut allergy? yes / no" can be asked, which is the specific way this feature crosses Art. 9 in practice: not by argument, but because it sounds administrative. The one extra answer is a boolean packed-lunch headcount with no room to elaborate, switched on per form. Every form tells the family, in the same words to the school in the builder, that medical, dietary and care information goes to the school office, which already holds it. **The reading risk** is scoped server-side in one module with a query per reader (`src/lib/consentForms.ts`): a parent sees their own child's form and their own household's answer and no tally; a class teacher sees the register for a class they hold; the school office sees **counts per class and no child's name**, which is the edge of rule 5's administrative-records clause and is asserted as such. One answer per child per form is a unique index, not a screen. The answer is never in the audit log — the log records that a family answered, and the register is where answers are read. The operator reads none of it (`ConsentResponse` is CREDENTIAL_NEVER, with its own gate fixture). No child has any route to a form. | **Low.** Lower than R19, and for a reason worth stating: R19 stays Low–Medium because a message body is free text and nothing can stop an adult typing anything into it. Here there is nothing to type into. The residual is that a school's *question* is free text and an incautious one could name a condition ("children with asthma should bring an inhaler") — the form goes to a whole class, the builder says not to name a child in it, and no answer records anything about an individual child either way. |
...
| R19 | Keep parent messaging off by default and school-switched; keep the office-hours hold two-way and override-free; keep the three message tables in the operator gate's strictest class and queryable from one module only | Sustained | Founder | Ongoing — each is a blocking test |
| R19 | Put the parent-facing wording (not for emergencies; who can read it; when a held message arrives) in front of pilot parents in the usability kit, and ask a pilot school's DSL whether "close the conversation" is enough or whether a designated-lead read with a recorded reason is needed — a separate amendment if so | Reduced | Founder | During the pilot phase |
| R19 | Say in the privacy notice for families, in the same plain English as the rest of it, that a parent can now write to the teacher, who can read it, and that nothing is emailed | Transparency | Founder | Before a real parent can send one |
| R20 | Keep the consent answer set in code and keep every response field non-free-text; a school-authored answer label is refused on sight, and is recorded as refused in the Part B plan and in `SAFEGUARDING.md` rule 22 so nobody re-derives it | Sustained | Founder | Ongoing — a blocking test |
| R20 | Say in the privacy notice for families that a school can ask for permission through StoryJar, that the answers are fixed, and that medical and dietary information is not asked for or kept here | Transparency | Founder | Before a real parent answers one |
