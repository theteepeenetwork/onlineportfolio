...
| 2026-08-17 | D5 | **School deletion does not exist in v1.** The handbook default, chosen. It is gated on R12 regardless: no deletion before a rehearsed restore. **Still true on 8 September 2026, and now stated on a screen rather than by absence.** A school can CLOSE its account (`src/app/actions/closure.ts`): the instruction is recorded with a date and a name, every member of staff goes back to their own free plan keeping the classes they brought, and the plan is frozen. **Nothing is deleted**, the card says so in words, and the erasure follows RETENTION.md's schedule by hand. Closing is not the deletion this row refuses, and the two must not be conflated: an `eraseSchool` still needs R12 satisfied (`docs/restore-rehearsal.md`) and still inherits F26 and F27. | OWNER, took the default |
...
| 2026-08-17 | D10, revisited | **Answered: rule 20 and rule 6a are merged** (#132), together with `docs/exceptional-access.md`. The owner asked what oversight of children's content there should be, which surfaced the case the design did not handle: every safeguarding route ends at the school, so a concern about the school closes the loop on itself. Rule 20 keeps the product blind and now names its own edge; the break-glass procedure governs the infrastructure access that already existed and was previously undefined. Rule 6a took the narrower wording. | OWNER, explicit approval |
...
| 2026-09-01 | Library publishing | **Publishing to the shared activity library is NOT an operator operation and is not in the registry.** The owner asked for ops accounts to author templates and publish them. Authoring needs the canvas, `ActivityTemplate` payload reads and filesystem writes, and the blindness gate refuses all three under its roots — `DrawingCanvas.tsx` fails `OPS-MEDIA-ELEMENT` before you reach the rest — so building it in `/ops` meant four widenings of the strictest gate in the repository to host a job that touches no school and no child's data. PR #137 had already shipped the alternative and named it: `School.canPublishToLibrary`, true for StoryJar Academy alone, settable only by `scripts/ops/seed-academy.mjs` or a migration, with no screen that writes it. So publishing lives on the school surface, in an Academy teacher account, on the same canvas a teacher uses, and the console gets **two read-only screens instead**: `/ops/academy`, which reads nothing at all and derives the sixteen sign-in addresses and codes from the seed's own scheme, and `/ops/library`, which uses the projection `good-ops-shared-activity-count.txt` already certified — title, slug, visibility, order, and a `_count` of the copies relation. No allowlist entry, no model reclassification, no registry row. The cost, stated rather than buried: `shared-activities.spec.ts` no longer asserts that NO code under `src/` writes `SharedActivity`, because one module now does. See `docs/library-publishing.md` for what replaced it. | OWNER, with the credential question put back to the owner and answered |
| 2026-09-01 | Academy codes on screen | **`/ops/academy` prints the sixteen Academy pupil sign-in codes, and this is the decision to do so.** Raised by the Rule 1 review, which is right that it deserves a dated row rather than a code comment. Handbook R11 forbids an operator reading a child's class code, and the blindness gate denies that column by name — this screen does not read it, it **recomputes** it from the seed's own formula, which reaches the same value by a route the gate cannot see. That is the honest description and the reason it is written down. What makes it acceptable: StoryJar Academy is StoryJar's own fictional tenant, seeded by `scripts/ops/seed-academy.mjs`, with no real child in it; the screen is behind the operator door and its TOTP; and the codes were already published in `docs/TEST_LOGINS.md`, which is a public repository. **What it must never become a precedent for**: the same reasoning applied to a real school would be a straightforward R11 breach, and nothing in the code marks the Academy as fictional — the safety here rests on a fact about a seed script, not on an invariant. If a second school ever gains `canPublishToLibrary`, this screen must be revisited before it is. The owner asked for the class logins by name, which is why the more protective option (drop the column, read the code inside the Academy teacher account like a teacher does) was not taken. | OWNER, asked for explicitly |
...
| ~~D10~~ | ~~Rule 20 wording for `SAFEGUARDING.md`, including the section 7 limitation sentence.~~ **Answered 2026-08-17, merged in #132.** | Done. The remaining related gap is the off-box audit copy, which `docs/exceptional-access.md` names as its own biggest weakness. |
