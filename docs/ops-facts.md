...
| 4.22 | `src/lib/erasure.ts`, the target of the PR7 refactor | Handbook §5 PR7, Brief 01 | ABSENT | Erasure logic currently lives in `src/lib/media.ts` (`deleteMediaFiles`) with callers in `src/lib/drafts.ts`, `src/app/actions/roster.ts`, `src/app/actions/classes.ts` and `src/app/actions/journal.ts`. PR7 is a refactor into a file that does not yet exist |
...
| 4.27 | `deleteOrphanedParents()` enforces the last-link cascade on all three paths | `RETENTION.md`, relied on by Amendment A3 | CONFIRMED | Defined in `src/lib/familyLinks.ts`, called from `src/app/actions/roster.ts`, `src/app/actions/classes.ts` and `src/app/actions/familyAccess.ts` |
