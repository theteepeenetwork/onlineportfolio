...
| 1.12 | `robots.txt` and a sitemap, which ops routes must be absent from | Handbook §6 item 7, R18 | Was ABSENT; a robots.txt now exists and the requirement is met | Updated 29 Aug 2026. `src/app/robots.ts` serves one, so the staging deployment can be kept out of the search index (it holds fixture children's work and an open operator door whose password is committed here as an R6 fixture). The definition-of-done line now bites on a file that exists, and it is satisfied: the file names **no path at all** — the disallow is a bare `/`, and `/ops` stays out of the index by the `X-Robots-Tag` header in `next.config.ts`, because naming the path in a world-readable file would publish it. Enforced by `ops-auth.spec.ts` and `ops-mail.spec.ts`, which both read the file and assert `/ops` is absent from it. There is still no `sitemap.xml` and no `sitemap.ts`, and both specs still assert the 404 |
...
| # | Claim | Source | Verdict | Detail |
| --- | --- | --- | --- | --- |
| 8.1 | `SAFEGUARDING.md` rules run 1 to 19, so rule 20 is the next free number | Handbook §0, Amendment E2 | CONFIRMED when checked, now **superseded** | Was true: numbered items 1 to 19 under headings A to I. Rule 20 was taken on 2026-08-17 by the operator-blindness rule, and 6a was added beside rule 6. Rule numbers are permanent identifiers assigned in order of addition, not positions on the page, so the list no longer reads sequentially. The next free number is 21 |
...
| 8.10 | `COMPETITIVE_POSITIONING.md` records "Two-way parent messaging / DMs" as REJECT on positioning grounds | Amendment D1 | **WAS CONFIRMED; reversed 2026-09-07** | The verdict now reads "BUILT, on our terms": messages held to office hours the school sets, in neither direction outside them, never reaching a child. Recorded in `SAFEGUARDING.md` rule 21 and its amendment, `docs/pricing-decisions.md` (school-plan only) and `docs/DPIA.md` R18. Anything downstream that relied on 8.10 needs re-reading. |
| 8.11 | The pilot one-pager promises no parent messaging | Amendment D1 | ABSENT as a document; **promise withdrawn 2026-09-07** | No one-pager exists. `LAUNCH_PLAN.md` line 134 lists writing one as a to-do. The teacher-facing script in `COMPETITIVE_POSITIONING.md` that carried the promise now says the opposite: families can message, only in school hours the school sets. A one-pager written from here on must say that. |
