...
| F76 | Low | Child-facing / canvas structure | **A child can reorder a teacher's template pages.** The tray's hold-and-slide calls `onReorder` for every card and the canvas passes it to `movePageTo`; `allowPageStructure` gates only copying and the teacher's right-click menu, although its own comment says a child's page count "is the shape of what they hand in" and only the builder turns structure on. Nothing is lost — every page, object and question moves together, and Undo takes a move back — but the teacher receives the worksheet in the child's order. Found 2026-09-10 while giving added pages a cross. | **Open — an owner decision**, not a defect to fix quietly: whether a child may place their own added pages among the teacher's (probably the wish), and whether the teacher's pages keep their order | `pages.spec.ts` covers that a move keeps everything together and can be undone; nothing asserts who may move what |
