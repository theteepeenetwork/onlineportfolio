...
| F84 | Medium | Published promises / transparency (rules 3, 25) | **Four published promises say no child's work is seen before a teacher approves it, and the classroom board contradicts them.** `src/app/admin/Promises.tsx:37,42` ("Nothing is seen until an adult has seen it"; "nobody else can see it — … not another child"), `src/app/legal/safeguarding/page.tsx:12`, `src/app/legal/privacy/page.tsx:58-59` and `src/app/admin/Guide.tsx:190`. The "until approval" half has been inaccurate since rule 25's carve-out on 10 September 2026; the 12 September decision removing the look-first gate makes the "an adult has seen it" half inaccurate too, because a waiting piece's thumbnail can reach a room through a projected run page before any adult has opened it. A privacy notice and a promise to the controller that are wrong about who sees a child's work are a transparency problem, not a wording one. Found 2026-09-12 by the safeguarding review of the board-picker change. Not changed there, because the fix is published copy governed by `docs/brand-and-copy.md` and is the owner's to word | **Open** — owner decision | — |
