...
| F35 | ~~High~~ | Data residency | Volume backups were switched on 2026-08-17, and Railway's own DPA says its primary processing is in the United States, with backups "across multiple sites and regions" and none named. A backup is a complete copy of every child's photograph, drawing and voice note, and SAFEGUARDING rule 10 commits StoryJar to UK or EU storage. **Resolved: Railway confirmed in writing on 2026-09-05 to the owner that volume backups are stored in the same region as the service, EU West (Amsterdam).** Recorded in RETENTION.md and DPIA R6. | **Resolved** 2026-09-05 |
...
| F82 | Low | A11y (child-facing) / WCAG 2.5.7 | **Reordering pages by touch needs a drag, with no single-tap alternative.** WCAG 2.2 **2.5.7 Dragging Movements** (AA) asks that anything done by dragging can be done by a single pointer without it. A child moves their own page by holding and sliding its card; the teacher's builder also has a right-click menu with Move up / Move down, but a child's canvas has nothing, and a finger has no right click. A child who can tap but not hold-and-drag — a motor impairment, a switch, a stylus that slips — cannot put their pages in order. Found 2026-09-11 while fixing F81. | **Open, for an owner decision.** F81 built the alternative — "Move this page left / right" in the card's menu — and shows it only in a menu opened from the keyboard, so nothing on a child's screen changed for anybody using a finger. Showing the same two buttons in the menu a hold opens would close this; it is a visible change to a child's screen, and the "Hold and slide to move it" line would go, so it is the owner's call rather than a side effect of a keyboard fix | None yet. The repro: on a pupil's canvas with a page they added, hold its card; the menu that opens has no way to move it |
| F84 | Medium | Published promises / transparency (rules 3, 25) | **Four published promises say no child's work is seen before a teacher approves it, and the classroom board contradicts them.** `src/app/admin/Promises.tsx:37,42` ("Nothing is seen until an adult has seen it"; "nobody else can see it — … not another child"), `src/app/legal/safeguarding/page.tsx:12`, `src/app/legal/privacy/page.tsx:58-59` and `src/app/admin/Guide.tsx:190`. The "until approval" half has been inaccurate since rule 25's carve-out on 10 September 2026; the 12 September decision removing the look-first gate makes the "an adult has seen it" half inaccurate too, because a waiting piece's thumbnail can reach a room through a projected run page before any adult has opened it. A privacy notice and a promise to the controller that are wrong about who sees a child's work are a transparency problem, not a wording one. Found 2026-09-12 by the safeguarding review of the board-picker change. Not changed in that review, because the fix is published copy governed by `docs/brand-and-copy.md` and is the owner's to word. **Reworded 13 September 2026 on the owner's approval**: all four, plus the families' plain-English privacy page and the operator handbook, now say that families, other pupils and other staff see nothing before approval and name the one exception, a class teacher choosing to show work on their classroom board. The three live policies carry the new date | **Fixed** | `security/published-promises.spec.ts`, which fails on any of the old absolute promises and on a page that drops the board exception (verified to fail against the old families' page) |
...
## F35 · Backups exist now, and the written evidence points at the United States · High → Resolved 2026-09-05

**Resolved 2026-09-05.** The owner put the direct question below to Railway and
received the answer this finding said was the only thing that could close it:
**volume backups for this service are stored in the same location as the
service itself, EU West (Amsterdam).** That is a written answer from the
provider about backups specifically, not an inference from where the service
runs, and it is recorded in `RETENTION.md` (the backups row) and `docs/DPIA.md`
(R6). Rule 10 holds for backups as it does for the volume. The customer-facing
privacy notice has carried the confirmation since the day it arrived; this
finding and the `RETENTION.md` row were the two places that had not caught up,
which is why they said "not confirmed" for two days after it was. The section
below is kept as written, because the reasoning about why inference was not
enough is the part worth keeping.
...
### Amended 2026-09-07: one thing the role now does decide

Parent messaging (`SAFEGUARDING.md` rule 21) shipped with a per-staff switch,
`Teacher.mayMessageParents`, whose NULL default is read from the role: a
TEACHER or ADMIN may message families, a TA may not. So the sentence above —
`"TA"` appears six times and gates nothing — stopped being true on that date,
and the Guide card that says the two roles are identical was corrected in the
same change.

**This is not the deferred option B, and the difference matters.** B is
*restricting* a TA: taking away the approval queue, which needs the DPO and
Mark. Nothing a TA could already do has been taken away here. A brand-new
channel to a parent simply does not open for them by default — the more
protective default for a new adult-to-adult channel (rule 8) — and an admin
can grant it to a named person on the same ⋯ menu, so it is a per-person
decision the school makes rather than a hard property of the title.

**Option C is still the missing thing.** Rule 21 needed "somebody other than
the class teacher can be in this conversation", and `Class.teacherId` still
cannot express it, so messaging carries its own per-thread grant
(`MessageThreadShare`) instead of a class relationship. That is a second
feature working around the same singular column, which is evidence for C
...
## F59 · "Remove from school" does not remove access · Critical → Fixed 2026-08-29
...
## F66 · A class moves; what its old teacher holds does not · High → Fixed 2026-08-29
...
### Fixed, 29 August 2026

Both limbs, in the same change as F59, because they are one event.

**(a)** `src/lib/classHandover.ts` rotates the class code whenever a class
changes hands — on removal and on ordinary reassignment, an owner decision taken
knowing the cost, which is that a routine September handover now needs the
children told a new code.

**(b)** All seven sites require the class as a second scope: the two writes
(`updateTemplate`'s push into live runs, `setRunStatus`), the roster read on the
activity page, the edit banner, the calendar, the dashboard count, and the
`/uploads` snapshot branch — the last included not because it leaked but so the
rule has no exception to argue about later.
