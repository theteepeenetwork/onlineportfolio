...
//
// WHY THIS LIVES OUTSIDE src/lib/ops/ (PR5, gate ruling R2)
//
// It was first written at src/lib/ops/mailHmac.ts, which is the name ruling R1
// would suggest. The ops blindness gate refused this: any file that imports a
// module under the ops roots is scanned as ops code, and src/lib/mailSuppressionSync.ts
// needs to call mailAddressHmac() to HMAC each address before writing it — which
// means it was being scanned, found a mailSuppression.upsert(), and fired
// OPS-MUTATION-MODULE. Moving the vocabulary here breaks the transitive import
// without moving any database access: this module reads only the environment and
// touches no Prisma model. reads.ts imports it via the gate's ALLOWED_LOCAL_IMPORTS
// list (widened in the same commit).
//
