...
| **Parent–teacher messages** (added 2026-09-07, SAFEGUARDING rule 21): one conversation per child between the child's linked parents and named school staff, held as `MessageThread` (who holds it, whether the school has closed it, and a teacher's optional note to the admin when they pass a family on), `Message` (the text, who wrote it, when, and when it is delivered), and `MessageThreadShare` (a colleague given the thread). **Child data**: a message is about a named child and routinely names them, which is why the operator area cannot read any of the three tables. Text only — no media, so no files. The school's own settings — the switch, the hours, the closed days — hold no person and are listed with the school below. | The child exists at the school | Deleted with the child, the class, or the school by the same cascades as the jar (`onDelete: Cascade` from `Student`), and with a family space when its last link goes (`deleteOrphanedParents()`). **A conversation stays whole when one voice in it is erased:** a guardian or a member of staff whose row goes leaves their messages with the sender reference set to NULL, because half a thread is unintelligible and the record of what was said to a family is the school's. **A lapsed school plan deletes nothing:** every conversation stays readable to both sides and sending stops, per the 2026-09-07 decision; the 12-month frozen clock applies to it as to the jar. Never emailed, never exported to a child, never in a log: the audit trail records that a message was sent and by which role, not what it said. A thread the school has *closed* is kept, not deleted — closing is an access decision. Read marks (`readByParentAt`, `readByTeacherAt`) are one timestamp each, used for a badge and never shown to the other side. |
| **A school's parent-messaging settings** (`MessagingPolicy`, `OfficeHourWindow`, `OfficeHoursClosure`, and `Teacher.mayMessageParents`, added 2026-09-07). The switch, the zone, a weekday with two times, a closed date with the school's own short label for it, and per member of staff whether the school lets them message families. **No child and no parent in any of it**; the per-staff switch is adult staff data. | The school exists | Deleted with the school by cascade. The per-staff switch goes with the staff row. Nothing here is on a retention clock of its own: it is configuration, not a record about a person. |
...
| **Backups of the volume** holding `/data/prod.db` and `/data/media`. Railway volume backups, switched on 2026-08-17 on the Pro plan. **Before that date there were none, and this row said there were.** The correction is recorded here rather than quietly edited, because a school's data protection lead may have read the old wording. | **Daily backups kept 6 days; weekly backups kept 1 month.** Railway's published schedule, not an estimate. | Deleted by Railway on that schedule. **A record erased on request is therefore gone from the last backup holding it within about one month**, which is inside the 35 days this row previously promised. Three caveats a reader should have: wiping a volume deletes all of its backups, so these protect against corruption and mistaken deletion of data, not against destruction of the volume itself; a backup can only be restored into the same Railway project and environment; and **where the backups are held — confirmed 2026-09-07:** Railway confirmed to the owner, in answer to the direct question, that volume backups are stored in the same region as the service, **EU West (Amsterdam)**. Between 2026-08-17 and that date this row said the location was not confirmed and the earlier Amsterdam claim had been removed; that is left on the record (F35 in `FINDINGS.md`) so that a reader of the older wording can see what changed and why. |
...
within the backup window above (about one month), deletion record kept.
