...
| **Email delivery records held by Brevo.** For each message Storyjar sends: the **adult** recipient address (a parent's or staff member's), a timestamp, the subject line, and the delivery status (sent, delivered, deferred, bounced). **No child's name and no child content is ever in an email**, so none reaches Brevo. Listed here because rule 9 requires every category to have a retention line, including the ones a sub-processor holds rather than us. | The message was sent within the last month | Deleted by Brevo after **1 month**, its shortest available setting, applied to all senders. **Message previews are switched off**, so Brevo stores no copy of the message body: this matters specifically because a stored preview of a sign-in email would leave a live, working sign-in link sitting in Brevo's web interface. Brevo's account-level *anonymous email tracking* is on, so any open or click event it records is not associated with an individual, and **Storyjar cannot tell whether a particular parent opened an email**. Brevo is EU-hosted (France and Germany), listed on /legal/sub-processors and assessed in `docs/DPIA.md` (R14). |
