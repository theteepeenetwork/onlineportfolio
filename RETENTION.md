...
| Teacher-authored activity media — template background pages **and** quiz answer-option pictures (`quizJson` / `quizSnapshotJson`), **including pictures placed through the Claude connector on the teacher's behalf** (added 2026-08-21: page backgrounds, question extracts and answer pictures, written by the same `saveImageDataUrl` path a teacher's own upload uses and bounded to 2 MB per picture and 10 MB per activity). The writer changed; the category, the location and the access control did not. | Template/assignment exists | Deleted with the template/account like other teacher-authored template media; served only via the authorising `/uploads` route, never to parents |
...
| **StoryJar's own shared activity library** (`SharedActivity`) **and its media** (`.media-shared`, served as `/uploads/shared/...`). StoryJar's own published teaching content: a title, instructions, background pages, optional quiz and movable-object payloads, tags and a suggested age band. **It holds no personal data of any kind**: no child data, no teacher personal data, and no media authored by any teacher. It belongs to no school and no teacher: the table has no `teacherId` and no `folderId`. Published from the repository by `scripts/ops/publish-shared-activities.mjs`. | StoryJar chooses to offer it | **Retained until StoryJar removes it.** There is no retention clock, because there is nothing here to expire. Removing or unpublishing one **does not touch any teacher's copy**: adding an activity copies the row *and* the files, so a teacher's copy is independent, keeps its own media, and only loses the provenance pointer (`ActivityTemplate.sourceSharedActivityId`, `onDelete: SetNull`). A teacher's copy is retained under the teacher-authored template rows above, like anything else they made. |
...
| **Mail delivery counters and refused-address labels** (added 2026-08-17). Three tables StoryJar keeps itself. `MailCounter` is a per-day tally of send attempts by template and outcome, holding **no recipient, no domain, no subject and no body**: a row says "eleven sign-in emails were attempted on this day and two failed with a 5xx", and there is no route from it back to a family. `MailSuppression` holds one row per address the provider is refusing, keyed by an **HMAC-SHA256 under a key held outside the database**, never the address, because a plain hash of an email address is reversible by dictionary and is not a de-identification measure. `JobRun` records when the check against Mailjet last ran and how it went. | Operational value only | **13 months** for counters and job runs, so a year-on-year comparison of a school September is possible. Refused-address labels are **deleted 90 days** after the provider stops refusing that address, because a label for an address nobody is blocked from is a row about a person that answers no question. Both purges belong to the scheduled job that is still an open item at the foot of this document. |
| **Connector tokens** (`ApiToken`, added 2026-08-21). One row per bearer token a teacher has made so Claude can build activities in their library, holding a SHA-256 of the token and never the token, a teacher-chosen label, four characters of the token as a display hint, and the times it was made and last used. **Adult data only, and it is a credential rather than a record of anything.** The token reaches one teacher's activity templates and folders and nothing else — not a class, not a child, not a journal item, not the queue — because `src/lib/api/` queries no other model (see the comment block in `prisma/schema.prisma`). `lastUsedAt` is written at most once an hour, on purpose: a teacher needs to know whether an old token is still in use before revoking it, and an exact record of when a teacher works is not that. | The teacher keeps it | **Revoking is deleting** — there is no revoked-but-present state on any of these tables. The row goes when the teacher revokes it on their account page, when they disconnect the app that owns it, when a connector refreshes (an **OAuth access token lasts one hour** and its predecessor is deleted as the new one is issued, so an hourly-refreshing connector leaves one row behind rather than a year of them), and with the teacher by cascade. The hash therefore stops existing rather than being filtered out, which is both stronger and one fewer thing every future query has to remember. |
| **Connector grants and registered apps** (`OAuthGrant`, `OAuthClient`, added 2026-08-21). `OAuthGrant` is one teacher's decision to let one app in, holding a hash of the single-use authorization code, the PKCE challenge, the redirect URI it was authorised against, and a hash of the current refresh token. `OAuthClient` is an app that registered itself (RFC 7591): a name it chose and its redirect URIs, with **no teacher and no personal data at all** until somebody authorises it. | The teacher keeps the connection | The grant is **deleted** — not flagged — when the teacher disconnects the app on their account page, when a used authorization code is presented a second time (RFC 6749 §10.5: a replayed code means somebody else has it), and with the teacher by cascade. Deleting it takes its access tokens with it. A registration nobody ever authorised is **deleted after 30 days**, swept when the next app registers, because a name and a URL attached to no teacher answers no question. |
| **The school a teacher picked at signup** (`Teacher.urn`, added 2026-08-24). The DfE URN of the school a teacher selected from the register, stored **alongside** the free-text `Teacher.schoolName` and never instead of it. **Not a new category of personal data**: it identifies an institution, every character of it is published by the DfE under an open licence, and it says nothing about the teacher that `schoolName` did not already say less precisely. Null is the ordinary case — a teacher outside England, or one whose school is missing from a hand-run snapshot, types the name — and null is the honest answer rather than a gap. It is checked against the register before it is stored, because a URN that is not in the register is worse than none: null says "this teacher typed it", a made-up key is a join somebody will one day follow. | The teacher account exists | **Deleted with the teacher**, by the same cascade as every other column on that row. No separate clock and no separate deletion path: it is a field on an existing record, not a record of its own. Nothing is retained about the establishment itself when a teacher goes — the register is a separate table with no data subject (next row). |
| **The school register** (`Establishment`, added 2026-08-24). One row per open, primary-facing school in England, imported from the Department for Education's Get Information about Schools extract: a URN, the school's name, its postcode, its local authority, its phase and its town, and **nothing else**. **There is no person in this table.** It holds no child data, no teacher data and no parent data; there is no foreign key to any person and no route from a row here back to one. Every column in it is already published by the DfE in a file anyone can download, under the Open Government Licence v3.0. The extract is **imported, never called at runtime** — see `docs/DPIA.md`, which records that this adds no sub-processor. Rows arrive only through `npm run gias:import`, run by a person; nothing schedules it and no operator screen can write to it. | StoryJar chooses to offer a school picker | **Replaced wholesale on each import**, and **not subject to a retention clock, because there is no data subject.** A retention period exists to stop personal data outliving its purpose, and there is no personal data here to outlive anything: what a stale row costs is a teacher not finding their school, which is an accuracy problem rather than a retention one. That is why `/ops/health` shows when the register was last refreshed and how old the source file was — staleness on this table is made visible rather than timed out. A school that closes leaves the register at the next import, because the import replaces rather than updates. |
...
| **Backups of the volume** holding `/data/prod.db` and `/data/media`. Railway volume backups, switched on 2026-08-17 on the Pro plan. **Before that date there were none, and this row said there were.** The correction is recorded here rather than quietly edited, because a school's data protection lead may have read the old wording. | **Daily backups kept 6 days; weekly backups kept 1 month; monthly backups kept 3 months.** Railway's published schedule for all three tiers, not an estimate. The previous figure in this row ("35 days rolling") matched no tier: it has been removed and replaced with the real schedule. | Deleted by Railway on the schedule in the previous column. **A record erased on request is therefore gone from the last backup holding it within about 3 months** (the monthly tier), not within one month as this row previously stated. The erasure runbook accounts for this window. Three caveats a reader should have: wiping a volume deletes all of its backups, so these protect against corruption and mistaken deletion of data, not against destruction of the volume itself; a backup can only be restored into the same Railway project and environment; and **the geographic location of the backups is not yet confirmed** (see the open item below), so any claim that they stay in Amsterdam has been removed rather than repeated. |
...
within the backup window above (up to about 3 months for the monthly tier), deletion record kept.
